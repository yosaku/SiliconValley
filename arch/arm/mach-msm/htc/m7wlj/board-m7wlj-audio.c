...
{
	static uint32_t audio_i2s_on_table[] = {
	...
	ret = gpio_request(GPIO_I2S_TX_PCLK, "I2S_TX_PCLK");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
				GPIO_I2S_TX_PCLK);
		goto err;
	}
	ret = gpio_request(GPIO_I2S_TX_WS, "I2S_TX_WS");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
				GPIO_I2S_TX_WS);
		goto err;
	}
	ret = gpio_request(GPIO_I2S_TX_DIN, "I2S_TX_DIN");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
				GPIO_I2S_TX_DIN);
		goto err;
	...
	for (i = 0; i < ARRAY_SIZE(audio_i2s_on_table); i++) {
		gpio_tlmm_config(audio_i2s_on_table[i], GPIO_CFG_ENABLE);
	}
err:
	return ret;
}
...
static int msm_mi2s_rx_free_gpios(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(audio_mi2s_off_table); i++) {
		gpio_tlmm_config(audio_mi2s_off_table[i], GPIO_CFG_DISABLE);
	}
...
{
	static uint32_t audio_i2s_off_table[] = {
	...
	for (i = 0; i < ARRAY_SIZE(audio_i2s_off_table); i++) {
		gpio_tlmm_config(audio_i2s_off_table[i], GPIO_CFG_DISABLE);
	}
...
static void mi2s_gpio_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(audio_mi2s_table); i++) {
		gpio_tlmm_config(audio_mi2s_table[i], GPIO_CFG_ENABLE);
	}
...
static void ext_hp_gpio_init(void)
{
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(10), "hp_en");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
				PM8921_GPIO_PM_TO_SYS(10));
		return;
	}
	ret = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(10), &param);
	if (ret)
